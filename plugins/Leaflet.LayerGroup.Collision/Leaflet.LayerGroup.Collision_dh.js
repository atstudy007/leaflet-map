...
function extensions(parentClass) { return {
	...
	addLayer: function(layer) {
		//layer中没有'options'和layer.options中没有'icon'
		//if ( !('options' in layer) || !('icon' in layer.options)) {
		//	console.log("layer中没有'options'和layer.options中没有'icon'");
	...
	//type:layer._icon  / layer._tooltip
	_maybeAddLayerToRBush: function(layer) {
		//console.log("_maybeAddLayerToRBush");
		//console.log(layer);
...
//			console.log(layer._icon);
			...
			if ( ('options' in layer) && ('icon' in layer.options)) {
				...
				var box = this._getIconBox(layer._tooltip._contentNode);
				boxes = this._getRelativeBoxes(layer._tooltip._contentNode.children, box);
				boxes.push(box);
				this._cachedRelativeBoxes[layer._leaflet_id] = boxes;
			}
		...
		if ( ('options' in layer) && ('icon' in layer.options)) {
			...
			var LatLng = layer._tooltip._latlng;
		}

		boxes = this._positionBoxes(this._map.latLngToLayerPoint(LatLng),boxes);
